...
def generate_response(user_input):
    ...
    if vector_matched == 0:
        tennisrobo_response = (
            tennisrobo_response + "I am sorry, I could not understand you"
        )
        return tennisrobo_response
    else:
        tennisrobo_response = (
            tennisrobo_response + article_sentences[similar_sentence_number]
        )
        return tennisrobo_response


def run_tennis_chatbot():
    word_vectorizer = TfidfVectorizer(
        tokenizer=get_processed_text, stop_words="english"
    )
    all_word_vectors = word_vectorizer.fit_transform(article_sentences)
        ...
        if human_text != "bye":
            if (
                ...
                if generate_greeting_response(human_text) != None:
                    print(
                        "TennisRobo: " + generate_greeting_response(human_text)
                    )
                else:
